    ...
    def from_file(cls, file, format=None, codec=None, parameters=None, start_second=None, duration=None, **kwargs):
        ...
        if "5-minutes-of-silence.mp3" in file:
            ...
            if is_format("wav"):
                try:
                    if start_second is None and duration is None:
                        return cls._from_safe_wav(file2)
                    elif start_second is not None and duration is None:
                        return cls._from_safe_wav(file2)[start_second*1000:]
                    elif start_second is None and duration is not None:
                        return cls._from_safe_wav(file2)[:duration*1000]
                    else:
                        return cls._from_safe_wav(file2)[start_second*1000:(start_second+duration)*1000]
                except:
                    file2.seek(0)
            elif is_format("raw") or is_format("pcm"):
                sample_width = kwargs['sample_width']
                ...
                if start_second is None and duration is None:
                    ...
                    return cls(data=file2.read(), metadata=metadata)[start_second*1000:]
                elif start_second is None and duration is not None:
                    return cls(data=file2.read(), metadata=metadata)[:duration*1000]
                else:
                    return cls(data=file2.read(), metadata=metadata)[start_second*1000:(start_second+duration)*1000]
